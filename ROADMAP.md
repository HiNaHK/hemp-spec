...
Python版とRust版はいずれも、`specs/`配下のHEMP仕様本文を基準にし、validation資料とPre-Milestone 1で選定する共通IPC実現方式を使って検証します。

### 2.4 共通IPC実現方式

Python版を最初の正式実装にする方針とは別に、正式実装群で共通に使うIPC実現方式は言語共通の前提として扱います。

正式実装群で共通に実装し、相互接続確認の基準にする主要IPC実現方式は、subprocess stdio型です。

subprocess stdio型は、local endpoint型で接続できなかった場合のfallbackではありません。

Python版とRust版は、同じHEMP仕様本文に従い、subprocess stdio型をそれぞれ実装します。
...
Pre-Milestone 1では、HEMP Core、HEMP-side Adapter、Transport-side Adapter、Transport Endpoint／Transport／Communication Pathの責任境界を整理し、正式実装群で共通に使うIPC実現方式をsubprocess stdio型として整理します。
...
specs/transport-binding/transport-adapters/subprocess-stdio/
```

これらは、HEMP-side AdapterとTransport-side Adapterの責任境界、およびtransport型ごとの言語非依存ルールを整理するための文書です。

subprocess stdio型は、Host側が管理するEngine processから得られたstdin相当Transport I/Oとstdout相当Transport I/Oを、HEMPのTransport I/Oとして扱うtransport型です。

現在の主な次作業は、この責任境界とsubprocess stdio型を前提に、HEMP library実装contractを整理することです。
...
Pre-Milestone 1: Transport Adapter責任境界と共通IPC方式整理
...
### 5.2 Pre-Milestone 1: Transport Adapter責任境界と共通IPC方式整理
...
正式実装群で共通に使うIPC実現方式をsubprocess stdio型として整理する。
```
...
- 正式実装群で共有する主要IPC実現方式としてのsubprocess stdio型の整理
...
- transport failure、frame construction failure、HEMP failure、abortの境界。
- 正式実装群で共有する主要IPC実現方式をsubprocess stdio型とすること。
- subprocess stdio型はlocal endpoint型のfallbackではないこと。
```

残作業:

```text
- HEMP library実装contractを整理する。
- subprocess stdio型を前提に、正式実装群が相互接続できることを確認するための判断材料を整理する。
...
およびsubprocess stdio型を前提にした共通IPC実現方式が整理されている。
```
...
Rust版は、HEMP仕様本文を基準にし、validation資料およびPre-Milestone 1で選定した共通IPC実現方式を使って検証する。
...
- 同じ入力に対する仕様上の判断の一致確認
- 言語差による挙動差の整理
- Pre-Milestone 1で選定したIPC実現方式上での相互接続確認
...
仕様適合性が確認され、
validation上の矛盾がなく、
Pre-Milestone 1で選定したIPC実現方式上で相互接続できる状態になっている。
```
...
Transport Binding共通仕様、`adapter-architecture/`、および`transport-adapters/`は、Transport I/O、HEMP-side Adapter、Transport-side Adapter、transport型ごとの責任境界を整理するために参照します。

共通IPC実現方式は、`transport-adapters/subprocess-stdio/`に基づいて整理します。

### 6.2 複数実装の検証方針

Python版とRust版は、同じHEMP仕様本文を基準にし、validation資料およびPre-Milestone 1で選定した共通IPC実現方式を使って検証します。

両実装の挙動が分かれた場合は、HEMP仕様本文を基準に、差分の原因を仕様解釈、実装、validation、IPC実現方式に分けて確認します。

確認結果に応じて、実装、test、または仕様本文を修正します。

### 6.3 共通IPC実現方式

主要IPC実現方式は、言語ごとに別々に選びません。

正式実装群は、subprocess stdio型を実装し、相互接続確認を行います。

subprocess stdio型は、単一言語での初期実装容易性ではなく、正式実装群が共有できることを基準に採用します。
...
Python版とRust版はいずれも、HEMP仕様本文を基準にし、validation資料およびPre-Milestone 1で選定した共通IPC実現方式を使って検証します。
