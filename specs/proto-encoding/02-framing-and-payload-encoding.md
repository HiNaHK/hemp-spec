...
3. HEM Length Header value が、HEMP Core または適用される Transport Binding Profile が定義する相互運用上の length limit に違反しないことを確認する。
...
HEM Length Header value が、HEMP Core または適用される Transport Binding Profile が定義する相互運用上の length limit に違反する場合、受信側は宣言された Payload bytes の読み取りを試みる必要はない。
