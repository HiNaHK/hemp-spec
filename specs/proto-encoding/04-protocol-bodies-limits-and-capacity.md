...
`AgreementVersion.protocol` が SemVer 2.0.0 として valid だが、受信側が対応しない protocol version である場合は、Agreement failure とする。
...
`AgreementResult.message = ""` は valid であるが、diagnostic value のため、送信側は可能な限り non-empty message を送るべきである。
...
`AgreementReplyLimits.message = ""` は valid であるが、推奨しない。
