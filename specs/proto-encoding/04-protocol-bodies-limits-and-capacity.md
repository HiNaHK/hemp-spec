...
optional bool result = false:
  presence があれば有効な scalar value である。

optional string reason = "":
  presence があれば有効な scalar value になり得る。

optional uint32 channel = 0:
  presence はあるが、semantic value として不正になり得る。
```
...
`AgreementVersion.protocol` が SemVer 2.0.0 に準拠していない場合は、HEM Body Contract failure とする。

`AgreementVersion.protocol` が SemVer 2.0.0 に準拠しているが、受信側が対応しない protocol version である場合は、Agreement failure とする。
...
`AgreementResult.result = false` は有効な scalar value であり、対応する agreement item が受け入れられなかったことを表す。

`AgreementResult.message` は人間向け説明である。
この field は機械判定の主入力として使用しない。

`AgreementResult.message = ""` は有効であるが、diagnostic value のため、送信側は可能な限り non-empty message を送るべきである。
...
`AgreementReplyLimits.message = ""` は有効であるが、推奨しない。
...
`PingReplyBody.result = false` は有効な scalar value であり、ping request が受け入れられなかったことを表す。
...
`ShutdownPostBody.reason = ""` は有効である。
...
`ShutdownReplyBody.result = false` は有効な scalar value であり、この HEMP session がその時点で normal shutdown を受け入れられなかったことを表す。
...
`CancelReplyBody.result = false` は有効な scalar value であり、cancel request が受け入れられなかったことを表す。
