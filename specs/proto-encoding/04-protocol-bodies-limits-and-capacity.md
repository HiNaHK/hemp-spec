...
optional bool result = false:
  presence があれば 有効な scalar value である。

optional string reason = "":
  presence があれば 有効な scalar value になり得る。

optional uint32 channel = 0:
  presence はあるが、semantic value として不正 になり得る。
```
...
`version`、`digest`、`limits` は presence が必須である。
...
`AgreementVersion.protocol` は presence が必須である。
...
受信側は `AgreementVersion.protocol` に対して、次を行ってはならない。
...
`AgreementVersion.protocol` が SemVer 2.0.0 に準拠していない な場合は、HEM Body Contract failure とする。

`AgreementVersion.protocol` が SemVer 2.0.0 に準拠している だが、受信側が対応しない protocol version である場合は、Agreement failure とする。
...
`AgreementVersion.application`、`AgreementApplicationVersion.host`、`AgreementApplicationVersion.engine` は presence が必須である。

`ApplicationIdentity.name` と `ApplicationIdentity.version` は presence が必須である。
...
`AgreementDigest.protocol` と `AgreementDigest.application` は presence が必須である。

`ChannelDigest.name` は presence が必須である。
...
受信側は `ChannelDigest.name` に対して、次を行ってはならない。
...
`ChannelDigest.digest` は presence が必須である。
...
`SessionLimits.hem_payload_length` は presence が必須である。

`SessionLimits.open_threads`、`SessionLimits.role`、`SessionLimits.protocol` は presence が必須である。

`OpenThreadsLimits.per_channel`、`OpenThreadsLimits.total` は presence が必須である。

`RoleLimits.post`、`RoleLimits.reply`、`RoleLimits.notice` は presence が必須である。

`RoleLimit.hem_count`、`RoleLimit.payload_total_length` は presence が必須である。

`ProtocolLimits.cancel` は presence が必須である。

`CancelProtocolLimits.in_flight_requests` は presence が必須である。
...
`AgreementReplyBody.version`、`AgreementReplyBody.digest`、`AgreementReplyBody.limits` は presence が必須である。

Agreement reply の version / digest の個別結果には `AgreementResult` を使用する。

`AgreementResult.result` と `AgreementResult.message` は presence が必須である。

`AgreementResult.result = false` は 有効な scalar value であり、対応する agreement item が受け入れられなかったことを表す。

`AgreementResult.message` は人間向け説明である。
この field は機械判定の主入力として使用しない。

`AgreementResult.message = ""` は有効 であるが、diagnostic value のため、送信側は可能な限り non-empty message を送るべきである。

`AgreementReplyVersion.protocol`、`AgreementReplyVersion.application`、`AgreementReplyApplicationVersion.host`、`AgreementReplyApplicationVersion.engine` は presence が必須である。

`AgreementReplyDigest.protocol`、`AgreementReplyDigest.application` は presence が必須である。

Agreement reply limits は、Agreement post body の `SessionLimits` 構造に対応する boolean leaf を持つ。

すべての boolean leaf は presence が必須である。

`AgreementReplyLimits.hem_payload_length`、`AgreementReplyLimits.open_threads`、`AgreementReplyLimits.role`、`AgreementReplyLimits.protocol`、`AgreementReplyLimits.message` は presence が必須である。

`AgreementReplyOpenThreadsLimits.per_channel`、`AgreementReplyOpenThreadsLimits.total` は presence が必須である。

`AgreementReplyRoleLimits.post`、`AgreementReplyRoleLimits.reply`、`AgreementReplyRoleLimits.notice` は presence が必須である。

`AgreementReplyRoleLimit.hem_count`、`AgreementReplyRoleLimit.payload_total_length` は presence が必須である。

`AgreementReplyProtocolLimits.cancel` は presence が必須である。

`AgreementReplyCancelProtocolLimits.in_flight_requests` は presence が必須である。

`AgreementReplyLimits.message` は limits 全体に対する人間向け説明である。
この field は機械判定の主入力として使用しない。

`AgreementReplyLimits.message = ""` は有効 であるが、推奨しない。
...
`PingReplyBody.result = false` は 有効な scalar value であり、ping request が受け入れられなかったことを表す。
...
`ShutdownPostBody.reason` は presence が必須である。

`ShutdownPostBody.reason = ""` は有効 である。

HEMP Core は `ShutdownPostBody.reason` の値の分類体系を定義しない。
...
`ShutdownReplyBody.result` は presence が必須である。

`ShutdownReplyBody.result = false` は 有効な scalar value であり、この HEMP session がその時点で normal shutdown を受け入れられなかったことを表す。

`ShutdownReplyBody.message` は presence が必須である。
...
`CancelPostBody.target` と `CancelPostBody.reason` は presence が必須である。

`CancelPostBody.reason = ""` は有効 である。

HEMP Core は `CancelPostBody.reason` の値の分類体系を定義しない。

`CancelTarget.channel` と `CancelTarget.thread` は presence が必須である。
...
ただし、`2147483648..4294967295` は semantic value として不正であり、Body Contract validation で拒否する。
...
`CancelReplyBody.result` は presence が必須である。

`CancelReplyBody.result = false` は 有効な scalar value であり、cancel request が受け入れられなかったことを表す。

`CancelReplyBody.message` は presence が必須である。
...
Agreement 成立前に、HEM Length Header value が `protocol_channel_payload_length_limit` を超える場合、受信側は Payload bytes の読み取りを試みず、HEMP framing failure とする。
...
この場合、受信側は宣言された Payload bytes の読み取りを試みる必要はない。

Agreement 成立前に、HEM Length Header value が `protocol_channel_payload_length_limit` 以下である場合、受信側は Payload bytes を読み取り、`hemp.v1.HemPayload` として decode し、Agreement body validation を適用する。
