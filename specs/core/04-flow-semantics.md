...
HEM Thread は、1つの channel 内で、HEM Timeline を順に扱うための識別単位である。
...
その post が Header validation および flow validation に成功し、論理 post 送信として state commit された後、その HEM Thread は closing 状態になる。

closing 状態は、`4.3 closed` の closed 移行条件をすべて満たすまで継続する。

closing 状態の HEM Thread では、新しい HEM Timeline を開始してはならない。

closing 状態の HEM Thread では、その closing 状態を発生させた close 要求 post に対応する reply を受信してよい。

この対応 reply は、通常の HEM Timeline reply として扱う。

closing 状態の HEM Thread では、closing 状態へ移行する前に有効に開始済みだった論理 notice 送信の継続 HEM を受信してよい。

closing 状態へ移行した後、新しい論理 notice 送信を開始してはならない。

### 4.3 closed

closing 状態の HEM Thread は、次をすべて満たした時点で closed 状態になる。

```text
- closing 状態を発生させた close 要求 post に対応する reply が、
  end = true かつ close = true で完了している。
- closing 状態へ移行する前に有効に開始済みだった論理 notice 送信が、
  すべて end = true または abort = true で終端している。
```

closed になった thread 番号は、同じ channel 内で再利用可能である。

closed 状態の HEM Thread に属する未完了の論理 post 送信、論理 reply 送信、または論理 notice 送信は存在してはならない。
...
ただし、HEM Thread が closing 状態へ移行する前に有効に開始済みだった論理 notice 送信については、その継続 HEM を closing 状態の HEM Thread 上で受信してよい。

この継続 HEM は、当該論理 notice 送信が `end = true` または `abort = true` によって終端するまで有効である。

closing 状態の HEM Thread では、新しい論理 notice 送信を開始してはならない。

対象 HEM Thread は、次の状態であってはならない。
...
notice を未open状態または closed状態の HEM Thread に紐づけた場合、HEMP flow violation とする。

closing 状態へ移行する前に有効に開始済みではない notice を closing 状態の HEM Thread に紐づけた場合、HEMP flow violation とする。
...
closed 状態の HEM Thread は、`open_threads.per_channel` の対象に含めない。

`open_threads.total` は、application channel 上で HEMP session 全体として同時に未closed状態にできる HEM Thread 数の上限である。

notice は HEM Thread を open しない。

notice-only channel で使われる notice 用 thread 番号は、`open_threads.per_channel` / `open_threads.total` の対象に含めない。
...
- 対応 post が存在しない reply を HEM Thread に紐づける
...
- reply を未open状態または closed状態の HEM Thread に紐づける
- closing状態を発生させた close 要求 post に対応しない reply を、closing状態の HEM Thread に紐づける
- notice を未open状態または closed状態の HEM Thread に紐づける
- closing状態へ移行する前に有効に開始済みではない新しい論理 notice 送信を、closing状態の HEM Thread に紐づける
- open 状態または closing 状態の thread 番号を、新しい HEM Thread の開始として使用する
- Agreement 済み open_threads.per_channel / total が許可する未closed状態の HEM Thread 数を超えて、新しい HEM Thread を開始する
```
