...
- Data Body Body Contract failure 条件
...
HEM Payload encoding において、通常 Data Body HEM は `application_body` を使用する。

```text
application_body bytes == data bytes fragment
```

`application_body` の bytes value が、その HEM で運ばれる Standard Data Body の data bytes fragment そのものである。

Standard Data Body は、`application_body` の内部に追加の wrapper、length field、type field、checksum、record delimiter、または nested message を定義しない。
...
`application_body` が empty bytes の場合、その data bytes fragment は empty である。empty fragment が有効かどうかは、この文書の `end` / `abort` 規則に従う。

HEM Payload encoding の具体的な field number、wire type、oneof branch mapping、optional presence、decode rule、schema 定義は、Protobuf Encoding specification で定義する。
...
HEM Payload encoding では、`abort = true` の application channel HEM は `abort_body` branch を使用する。
...
## 9. application_body / abort_body との関係

通常 Data Body HEM は、次の構造を持つ。
...
`abort_body` は、通常完了時の Standard Data Body data bytes fragment ではない。

Standard Data Body は、`abort_body` の内部形式、reason、message、diagnostic 情報を定義しない。
...
この条件に違反する場合は、Standard Data Body Body Contract failure ではなく、HEMP flow violation である。

HEM Payload encoding において、Body Contract 向けの導出容量として次を使う。

```text
max_core_envelope_length = 64 bytes
hem_body_capacity = max(0, body.limits.hem_payload_length - 64)
```

Standard Data Body は、この導出容量を自身の data bytes capacity として採用する。

```text
standard_data_body_data_capacity = hem_body_capacity
```

したがって、通常 Standard Data Body HEM では次を満たさなければならない。

```text
len(application_body) <= standard_data_body_data_capacity
```

これに違反する場合は、Header と flow が有効であれば Standard Data Body Body Contract failure とする。

`standard_data_body_data_capacity` は Standard Data Body の Body Contract 上限である。

`standard_data_body_data_capacity` は Core 共通の `application_body` bytes 強制上限ではない。
...
`chunking banned flag = true` の channel で `end = false` の HEM を受信した場合、これは Standard Data Body Body Contract failure ではなく、HEMP flow violation である。
...
- 各通常 Data Body HEM の len(application_body) が standard_data_body_data_capacity を超えない
...
1 HEM として表現した場合の encoded HEM Payload length または `len(application_body)` が、Agreement 済み limits または `standard_data_body_data_capacity` を満たさない場合、その data はその channel では送信できない。
...
これは、`application_body` が empty bytes であることを意味する。
...
受信側が、Standard Data Body の通常 HEM としてこれを受信した場合、Data Body Body Contract failure とする。
...
## 17. Body Contract failure 条件

### 17.1 適用対象

この章は、application channel 上で Standard Data Body として扱われる HEM の body branch と通常 data fragment 検証に適用する。

通常 Data Body body 検証は、次に対して適用する。

```text
application channel 上で Standard Data Body として扱われる HEM
かつ
abort = false
```

`abort = true` の HEM は、通常 Data Body body として検証しない。

ただし、application channel の `abort = true` HEM は `abort_body` branch を使用しなければならない。

body branch mismatch は HEM Body Contract failure である。

### 17.2 Data Body Body Contract failure となるもの

次の場合、Data Body Body Contract failure とする。

```text
- channel > 0, abort = false で body branch が application_body ではない。
- channel > 0, abort = true で body branch が abort_body ではない。
- 通常 Data Body HEM の len(application_body) が standard_data_body_data_capacity を超える。
- 通常 Data Body HEM の len(application_body) が 0 であり、end = false, abort = false である。
- 1つの Standard Data Body 論理送信内で、abort = false の通常 HEM が application_body 以外の body branch を使用する。
```

`application_body` bytes value が Standard Data Body の data bytes fragment である。

```text
application_body bytes == data bytes fragment
```

したがって、Standard Data Body には、`application_body` の内部構造不正という failure 条件はない。

`application_body` は任意の byte 列である。

ただし、Protobuf decode 不能、HEM Payload の header 欠落、known body branch 欠落、Core Header field の欠落または値不正は、この章の Data Body Body Contract failure ではなく、HEMP Protobuf Encoding および HEMP Core の validation layer に従って分類する。

### 17.3 Data Body Body Contract failure ではないもの

次は、Data Body Body Contract failure ではない。
...
- abort = true HEM の abort_body が通常 Standard Data Body body として解釈できないこと。
```
