...
この文書では、Transport Binding の位置づけ、HEMP Core と Transport Binding の責務分界、HEM frame と transport の関係、complete HEM frame の扱い、`direction` と実通信方向の一致、順序保証単位と HEMP Core flow semantics の関係、transport message payload / batching の扱い、HEMP session と Transport Binding instance の関係、transport failure、`abort` と transport failure の境界、および Standard Data Body と Transport Binding の関係を扱う。
...
- Engine -> Host の実通信方向
- 順序保証単位
- HEM frame を順序保証単位へ mapping する方法
- HEM frame bytes の運び方
- HEM Header の direction と実通信方向の対応
...
HEM frame sequence は、1つの順序保証単位の内部で運ばれる、1つ以上の complete HEM frame の列である。
...
`abort = true` は、HEM frame sequence の終端、順序保証単位の終端、transport stream の終端、または Transport Binding instance の終端を意味しない。

---

## 8. direction と実通信方向

Transport Binding は、HEMP session 内で使用する実通信方向を定義しなければならない。

Transport Binding は、少なくとも次の2つの実通信方向を定義する。
...
HEM Header の `direction` は、Transport Binding が定義する実通信方向と一致しなければならない。

```text
direction = to_engine:
  Host -> Engine の実通信方向で送信する。

direction = to_host:
  Engine -> Host の実通信方向で送信する。
```

### 8.1 送信側の責任

送信側は、HEM Header の `direction` と一致する実通信方向へ HEM frame を送信しなければならない。

Host は、`direction = to_engine` の HEM を Host -> Engine 方向へ送信する。

Engine は、`direction = to_host` の HEM を Engine -> Host 方向へ送信する。
...
受信側は、HEM frame を受信した実通信方向と、HEM Header の `direction` が一致していることを検証しなければならない。

Engine が Host -> Engine 方向で受信した HEM では、`direction` は `to_engine` でなければならない。

Host が Engine -> Host 方向で受信した HEM では、`direction` は `to_host` でなければならない。

一致しない場合、HEMP header validation failure とする。

### 8.3 実通信方向と物理 transport

実通信方向は、Transport Binding 上の論理的な通信方向である。

実通信方向は、物理 transport の本数と一致しなくてよい。

Host -> Engine と Engine -> Host は、1本の双方向 transport で実現してもよい。

または、2本の単方向 transport の組で実現してもよい。

または、複数の順序保証単位の集合で実現してもよい。

---

## 9. 順序保証単位と HEMP Core flow semantics

順序保証単位は、Transport Binding が HEM frame sequence を信頼性付き・順序付きで配送する単位である。

各順序保証単位は、Host -> Engine または Engine -> Host のいずれか一方の実通信方向に属する。

1つの順序保証単位の中に、Host -> Engine と Engine -> Host の両方向の HEM frame を混在させてはならない。

Transport Binding は、各実通信方向に1つ以上の順序保証単位を定義してよい。

異なる順序保証単位の間に、Transport Binding 基本モデルとしての単一グローバル順序は要求しない。

ただし、Transport Binding は、複数の順序保証単位を使う場合でも、HEMP Core flow semantics を満たす順序で complete HEM frame を HEMP Core へ渡さなければならない。

Transport Binding は、HEMP Core が定義する順序制約を緩和してはならない。

HEMP Core の順序制約には、少なくとも次を含む。

```text
- 同一の論理 post 送信に属する HEM は seq 昇順で処理される。
- 同一の論理 reply 送信に属する HEM は seq 昇順で処理される。
- 同一の論理 notice 送信に属する HEM は seq 昇順で処理される。
- reply は対応する論理 post 送信が end = true で完了した後に開始する。
- 同じ channel + thread 上では、現在の HEM Timeline の対応 reply が完了するまで次の post を開始しない。
- notice は、HEMP Core が定義する notice の thread 状態、direction、seq、未完了論理 notice 送信数の規則に従う。
- agreement、shutdown、cancel などの protocol channel は、HEMP Core が定義する使用可能タイミングと固有の flow rule に従う。
- aborted 状態で終端済みの論理 post 送信、論理 reply 送信、または論理 notice 送信に対して、同じ論理送信の継続 HEM を有効な継続として処理しない。
```

Transport Binding は、この文書に列挙していない HEMP Core 上の順序制約も変更してはならない。

複数の順序保証単位を持つ Transport Binding では、Transport Binding または HEMP 実装は、各順序保証単位から complete HEM frame を構成し、HEMP Core の順序制約を満たす形で HEMP Core の処理へ渡さなければならない。

複数の順序保証単位を持つ Transport Binding では、同一の論理送信に属する HEM を複数の順序保証単位へ mapping する場合でも、`abort = true` の HEM を含む seq 順序と論理送信終端状態が HEMP Core semantics に合うように、HEMP Core へ渡す前に保留、並べ替え、合流しなければならない。
...
1つの transport message payload に複数の HEM frame を含める場合、その transport message payload は1つの順序保証単位に属するものとして扱う。

その transport message payload 内のすべての HEM frame は、その順序保証単位が属する実通信方向で受信されたものとして扱う。

したがって、同じ transport message payload 内の各 HEM Header の `direction` は、その transport message payload が属する順序保証単位の実通信方向と一致しなければならない。
...
- 順序保証単位をいくつ定義するか。
- 各順序保証単位が属する実通信方向。
- HEM frame sequence を順序保証単位上でどう運ぶか。
- HEM frame をどの順序保証単位へ mapping するか。
- protocol channel の HEM をどの順序保証単位へ mapping するか。
- application channel の HEM をどの順序保証単位へ mapping するか。
- 複数の順序保証単位から受信した HEM frame を HEMP Core へ渡す前にどう保留、並べ替え、合流するか。
...
Transport Binding は、`abort = true` の HEM を通常の HEM frame としてどの順序保証単位へ mapping するかを定義してよい。
...
Transport Binding は、`abort = true` を transport reset、stream reset、transport close、順序保証単位の終了、または transport failure として再定義してはならない。
...
Transport Binding instance は、1つ以上の順序保証単位の集合として構成されてよい。
...
複数の順序保証単位を持つ場合でも、その Transport Binding instance が同時に運ぶ HEMP session は1つだけである。
...
### 13.3 Agreement 成立前の順序保証単位

Agreement 成立前でも、Transport Binding は HEM frame を運ぶ。

ただし、Agreement 成立前に有効な HEM は `agreement` post / reply に限られる。

複数の順序保証単位を持つ Transport Binding でも、Agreement 成立前に application channel 用の順序保証単位で application channel の HEM を送信してはならない。

Agreement 成立前に HEM を送信できる順序保証単位は、`agreement` を送受信するために Transport Binding が定義したものに限る。
...
## 14. 複数の順序保証単位と session-level control

複数の順序保証単位を持つ Transport Binding でも、`shutdown` は HEMP session 全体に対する protocol channel である。

Transport Binding は、`shutdown` post を特定の順序保証単位だけの終了要求として扱ってはならない。

Transport Binding は、`shutdown` post が、同じ HEMP session 内の他の順序保証単位上の HEM とどのような順序関係を持つかを定義しなければならない。

Transport Binding は、`shutdown` post を送信する前に送信側が全順序保証単位を終了可能な状態にする規則、または `shutdown` を session-level barrier として扱う規則を定義してよい。
...
複数の順序保証単位を持つ Transport Binding でも、`cancel` request を対象 HEM Timeline と同じ順序保証単位で運ぶことは必須ではない。

ただし、`cancel` request が対象 HEM Timeline とは異なる順序保証単位で運ばれる場合、受信側では `cancel` request が対象 HEM Timeline の開始 HEM より先に処理される可能性がある。
...
`cancel` を受理した結果として対象 reply が `abort = true` で終端する場合でも、その `abort = true` の HEM は対象 application channel の実際の順序保証単位上で通常の HEM frame として運ばれなければならない。
...
transport failure は、下位 transport、Transport Binding instance、順序保証単位、または Transport Binding 実装に起因して、HEMP session を運べなくなる失敗である。
...
複数の順序保証単位を持つ Transport Binding では、いずれかの順序保証単位の失敗によって、その Transport Binding instance 上の HEMP session を正しく運べなくなった場合、現在の HEMP session は継続不能として扱う。
...
transport failure は、下位 transport、順序保証単位、または Transport Binding instance の失敗である。
...
Transport Binding は、transport close、stream close、FIN、RST、I/O error、timeout、process 終了、または順序保証単位の予期しない終了を、`abort = true` の HEM として合成してはならない。

Transport Binding は、`abort = true` の HEM を送信するために transport close、stream close、FIN、RST、または順序保証単位の終了を要求してはならない。
