...
この文書では、channel の通信レーン性質と digest 計算対象を定義する。
...
Channel Table は、channel の通信レーン性質を定義する Table である。
...
Protocol Channel Table は、各 protocol channel の基本的な通信レーン性質を定義する。
...
派生仕様または独自拡張仕様では、別の protocol digest name を定義してよい。
