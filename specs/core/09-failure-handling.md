...
Standard Data Body を使う application channel では、Standard Data Body の Body Contract failure 条件に従う。
...
## 14. Agreement failure と Protocol Error
...
Agreement failure 時、現在の HEMP 通信状態は終了状態として扱う。
...
これらの failure が発生した場合、現在の HEMP 通信状態は失敗状態として扱う。
...
`agreement` の Body Contract failure では、Header が有効で reply 可能な場合、`agreement` reply body を使って failure を返してよい。
...
Header が有効で、対応 reply を返せる状態であれば、各 protocol channel の専用 reply body で `result = false` を返してよい。
...
Standard Data Body の Body Contract failure は、`08-data-body-rules.md` に従う。
