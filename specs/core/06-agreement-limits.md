...
`agreement` reply 完了後、`channel = -1 / thread = 1` は closed になる。
...
`agreement` post body の標準 semantic field tree は次の通りである。
...
`agreement` reply body の標準 semantic field tree は次の通りである。
...
Host-Engine Agreement は、次をすべて満たす場合だけ成立する。
...
body.limits 内の判定boolean leaf がすべて true
```

1つでも条件を満たさない場合は Agreement failure とする。

---

## 20. Agreement failure

Agreement 成立条件を1つでも満たさない場合は Agreement failure とする。

Agreement failure は Protocol Error ではない。
...
agreement body が Body Contract として読めない、判定できない、または受け入れられない場合は、Agreement Body Contract failure として扱う。

Header が有効で agreement reply を返せる場合、Engine は agreement reply body を使って failure を返してよい。

Body Contract として読めない、判定できない、または受け入れられない Agreement 項目は、対応する Agreement 結果を failure として返してよい。
...
Body Contract failure に対して `result = false` または判定 boolean leaf `false` を返す場合、`message` に人間向け説明を入れる。
...
`agreement` post body の `limits` は次を持つ。
...
`hem_body_capacity` は Core 共通の application body bytes 強制上限ではない。

Body Contract がこの容量を採用するかどうかは、対応する Body Contract が定義する。

Standard Data Body は次を採用する。
...
`body.limits.open_threads.per_channel` は、1つの application channel 内で同時に open 状態にできる HEM Thread 数の上限である。

`body.limits.open_threads.total` は、application channel 上で HEMP session 全体として同時に open 状態にできる HEM Thread 数の上限である。
...
application channel において `multi-thread banned flag = false` の場合、`body.limits.open_threads.per_channel` と `body.limits.open_threads.total` に従って、複数の open HEM Thread を許可する。

notice は HEM Thread を open しない。

notice で使われる thread 番号は、`body.limits.open_threads.per_channel` / `body.limits.open_threads.total` の対象に含めない。

`open_threads.total` は、thread を open した側が Host か Engine かを問わず、すべての application channel 上の open HEM Thread を合算して数える。
...
`body.limits.protocol.cancel.in_flight_requests` は、application channel 上の open HEM Thread 数には含めない。
