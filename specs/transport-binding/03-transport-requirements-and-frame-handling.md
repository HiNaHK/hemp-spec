...
frame construction failure の原因が lower transport の close、EOF、I/O error、timeout、reset、process termination、resource exhaustion、ordering unit continuation failure、または communication path continuation failure である場合、Transport Binding instance は transport failure により継続不能であり、現在の HEMP session も継続不能として扱う。
