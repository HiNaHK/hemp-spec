...
ただし、transport message payload 全体を valid HEM frame sequence として正しく切り出せない場合、receiver はその transport message payload 内の HEM を処理しない。

この切り出し処理は、HEMP Core の transaction mechanism ではない。

transport message payload 全体を valid HEM frame sequence として正しく切り出せた後、message 内の HEM frame は出現順で処理する。
